...
class Laporan_grafik_peternakan extends CI_Controller
{
	...
	
	{
        ...
        $this->load->model("model_laporan_grafik_peternakan");
...
public function index()
	{
		$data = array(
			    'nama_pasar'	=> $this->model_laporan_grafik_peternakan->get_nama_pasar(),
		);
		// echo "<pre>";print_r($data['nama_pasar']);die;
		$this->load->view('frontend/laporan_grafik_peternakan', $data);
    }
    public function detail()
	{
		...
		$data['hasil'] = $this->model_grafik_peternakan->get_data($search);
		if (empty($data['hasil'])) {
			$this->session->set_flashdata('userdata', 'gagal');
			redirect('laporan_grafik_peternakan');
		}
		$data['rs_search'] = $search;
		$count_data = count($data['hasil']);
		$jumlah_harga= $this->model_grafik_peternakan->get_jumlah_harga($search);
