...
class Laporan_grafik_perpang extends CI_Controller
{
	...
	
	{
        ...
        $this->load->model("model_grafik_perpang");
        $this->load->model("model_laporan_grafik_perpang");
...
public function index()
	{
		$data = array(
			    'nama_pasar'	=> $this->model_laporan_grafik_perpang->get_nama_pasar(),
		);
		// echo "<pre>";print_r($data['nama_pasar']);die;
		$this->load->view('frontend/laporan_grafik_perpang', $data);
    }
    public function detail()
	{
		...
		$data['hasil'] = $this->model_grafik_perpang->get_data($search);
		if (empty($data['hasil'])) {
			$this->session->set_flashdata('userdata', 'gagal');
			redirect('laporan_grafik_perpang');
		}
		$data['rs_search'] = $search;
		$count_data = count($data['hasil']);
		$jumlah_harga= $this->model_grafik_perpang->get_jumlah_harga($search);
		...
		$this->load->view('grafik_perpang', $data);
    ...
    public function cari()
	{
		...
		redirect("Laporan_grafik_perpang/detail");
	}
	}